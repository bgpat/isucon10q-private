...
func initialize(c echo.Context) error {
	...
	if err := load(nrctx(c)); err != nil {
		c.Logger().Errorf("Load error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func getChairDetail(c echo.Context) error {
	...
	err = db.GetContext(nrctx(c), &chair, query, id)
	if err != nil {
...
func postChair(c echo.Context) error {
	...
	for _, row := range records {
		...
		_, err := tx.ExecContext(nrctx(c), "INSERT INTO chair(id, name, description, thumbnail, price, height, width, depth, color, features, kind, popularity, stock) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)", id, name, description, thumbnail, price, height, width, depth, color, features, kind, popularity, stock)
		if err != nil {
...
func buyChair(c echo.Context) error {
	...
	var chair Chair
	err = tx.QueryRowxContext(nrctx(c), "SELECT * FROM chair WHERE id = ? AND stock > 0 FOR UPDATE", id).StructScan(&chair)
	if err != nil {
		if err == sql.ErrNoRows {
	...
	_, err = tx.ExecContext(nrctx(c), "UPDATE chair SET stock = stock - 1 WHERE id = ?", id)
	if err != nil {
...
func getLowPricedChair(c echo.Context) error {
	...
	err := db.SelectContext(nrctx(c), &chairs, query, Limit)
	if err != nil {
		if err == sql.ErrNoRows {
...
func getEstateDetail(c echo.Context) error {
	...
	var estate Estate
	err = db.GetContext(nrctx(c), &estate, "SELECT * FROM estate WHERE id = ?", id)
	if err != nil {
		if err == sql.ErrNoRows {
...
func postEstate(c echo.Context) error {
	...
	for _, row := range records {
		...
		_, err := tx.ExecContext(nrctx(c), "INSERT INTO estate(id, name, description, thumbnail, address, latitude, longitude, rent, door_height, door_width, features, popularity) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)", id, name, description, thumbnail, address, latitude, longitude, rent, doorHeight, doorWidth, features, popularity)
		if err != nil {
...
func searchEstates(c echo.Context) error {
	...
	if c.QueryParam("rentRangeId") != "" {
		rent, err = getRange(estateSearchCondition.Rent, c.QueryParam("rentRangeId"))
		if err != nil {
	...
	if doorHeight == nil && doorWidth == nil && rent == nil && features == nil {
...
func getLowPricedEstate(c echo.Context) error {
	...
	err := db.SelectContext(nrctx(c), &estates, query, Limit)
	if err != nil {
		if err == sql.ErrNoRows {
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	err = db.SelectContext(nrctx(c), &estates, query, w, h, w, d, h, w, h, d, d, w, d, h, Limit)
	if err != nil {
		if err == sql.ErrNoRows {
			return c.JSON(http.StatusOK, EstateListResponse{[]Estate{}})
		}
...
func searchEstateNazotte(c echo.Context) error {
	...
	err = db.SelectContext(nrctx(c), &estatesInBoundingBox, query, b.BottomRightCorner.Latitude, b.TopLeftCorner.Latitude, b.BottomRightCorner.Longitude, b.TopLeftCorner.Longitude)
	if err == sql.ErrNoRows {
	...
	for _, estate := range estatesInBoundingBox {
		...
		err = db.GetContext(nrctx(c), &validatedEstate, query, estate.ID)
		if err != nil {
			if err == sql.ErrNoRows {
				continue
			} else {
...
func postEstateRequestDocument(c echo.Context) error {
	...
	err = db.GetContext(nrctx(c), &estate, query, id)
	if err != nil {
		if err == sql.ErrNoRows {
			return c.NoContent(http.StatusNotFound)
		}
